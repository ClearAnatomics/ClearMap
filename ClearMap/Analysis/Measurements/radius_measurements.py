...
Measures intensity decays at specified points in the data.
"""
...
def measure_radius(source, points, fraction=None, value=None,
                   ...
                   return_indices=False, processes=None, verbose=False):
    """
    ...
    fraction : float or None
        Fraction of center intensity that needs to be reached to determine the
        radius.
        If None, value needs to be given.
    value : array or float or None:
        The value below which the intensity has to fall to measure the radius
    ...
    return_radii_as_scalar : bool
        If True, return the radii as single floats, otherwise a radius for
        each dimension.
    return_indices : bool
        If True, return the indices of the search which allows identifying the
    ...
    indices : array
        Array of measured indices at which the radius detection condition is met.
    """
    if scale is None:
        warnings.warn(f'No scale given, using 1 for all dimensions. '
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Measuring radii of {points.shape[0]} points in array of shape {source.shape}.')

    ndim = source.ndim
    if not hasattr(max_radius, '__len__'):
        max_radius = [max_radius] * ndim
    if len(max_radius) != ndim:
        raise ValueError(f'The maximal search radius {max_radius} has wrong dimension!')
    ...
    if method == 'sphere':
        ...
        raise ValueError(f'The method is not "sphere" or "rectangle" but {method}!')

    worker_args = [source, points, search] + [value if value is not None else fraction]
    worker_kwargs = {'sink': None, 'processes': processes, 'verbose': verbose}
    if value is not None:
        if hasattr(value, '__len__'):
            measured = mpl.find_smaller_than_values(*worker_args, **worker_kwargs)
        else:
            measured = mpl.find_smaller_than_value(*worker_args, **worker_kwargs)
    elif fraction is not None:
        measured = mpl.find_smaller_than_fraction(*worker_args, **worker_kwargs)
    else:
    ...
    if return_radii:
        ...
        if return_radii_as_scalar:
            ...
            distance_table = np.hstack([distance_table, default])
        else:
            distance_table = np.vstack([distance_table, [default] * ndim])
        radii = distance_table[measured]
        result += (radii,)
    if return_indices:
