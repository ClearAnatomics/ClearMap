...
from ClearMap.Script.Environment import *  # analysis:ignore
...
ws = wsp.Workspace(name='TubeMap', directory=directory);
...
if debug:
    ...
    slicing = (slice(None), slice(0, 600), slice(900, 1200));
...
processing_parameter.update(processes=15,
                            as_memory=True,
                            verbose=True);
...
postprocessing_processing_parameter = vasc.postprocessing_processing_parameter.copy();
postprocessing_processing_parameter.update(size_max=50);
...
vf.fill_vessels(source, sink, resample=1, threshold=0.5, cuda=True, processing_parameter=processing_parameter,
                verbose=True)
...
io.write(ws.filename('binary', postfix='full'), np.logical_or(io.read(ws.filename('binary', postfix='arteries_filled')),
                                                              io.read(ws.filename('binary', postfix='filled'))))

# %% Smoothing and filling - arteries

source = ws.filename('binary', postfix='full');
...
radii, indices = mr.measure_radius(ws.filename('binary', postfix='final'), coordinates,
...
ano.set_annotation_file('/home/sophie.skriabine/Documents/ClearMap_Ressources/Andromachi_annotation_L.tif')
...
distance_atlas = io.read(distance_file);
...
vein_large_radius = 8
vein_acta2_expression_min = 0;
vein_acta2_expression_max = 2800;

radii = graph.edge_property('radii');
acta2_expression = graph.edge_property('artery_raw');

vessel_large = radii >= vein_large_radius;

vein_expression = np.logical_and(acta2_expression >= vein_acta2_expression_min,
                                 acta2_expression <= vein_acta2_expression_max);
...
grt = grp.load(ws.filename('graph', postfix='reduced_transformed'))
...
gs = grt.sub_slice((slice(1, 300), slice(50, 480), slice(165, 185)), coordinates='coordinates_atlas');
...
voxelize_branch_parameter = {
    ...
    "radius": (15, 15, 15),
    "weights": None,
    "shape": io.shape(template_file),
    "verbose": True
};

vertices = grt.vertex_coordinates();
