...
The module aligns stacks allowing them to wobble around a wobble axis, i.e.
due to oscillatory movements during image acquisition.
"""
...
class WobblySource(strg.Source):
    ...
    def wobble(self, wobble):
        if wobble.shape[0] != self.height or wobble.ndim != self.wdim:
            raise ValueError(f'Number of wobbles {wobble.shape[0]:d} is not equal the number of planes={self.height:d}'
                             f' along the wobble axis {self.axis:d}!')
        self._wobble = np.array(wobble, dtype = int)
    ...
    def coordinate_to_local(self, coordinate):
        """Converts a wobble axis coordinate to a local coordinate wrt to the sources' origin.
        ...
        if coordinate < position or coordinate >= position + shape:
            raise RuntimeError(f'Coordinate {coordinate:d} out of range ({position:d},{position + shape:d})!')
    ...
    def coordinate_from_local(self, local_coordinate):
        ...
        if local_coordinate < 0 or local_coordinate >= shape:
            raise RuntimeError(f'Coordinate {local_coordinate:d} out of range ({position:d},{position + shape:d})!')
    ...
    def _wobble_to_position(self, wobble, coordinate):
        ...
        -------
        position : tuple
          The position with added wobble.
        """
        axis = self.axis
...
class WobblyAlignment(strg.Alignment):
    ...
    def __init__(self, pre = None, post = None, shifts = None, displacements = None, qualities = None, status = None, axis = 2,  shift = None, displacement = None, quality = None):
        ...
        #overlap region
        overlap = strg.overlap(strg.Region(position = pre.position[axis:axis+1], shape = pre.shape[axis:axis+1]),
    ...
    def displacements(self, value):
        if len(value) != self.upper_coordinate - self.lower_coordinate:
            raise ValueError(f'Dimension mismatch {len(value):d} != {self.upper_coordinate - self.lower_coordinate:d}')
    ...
    def overlay_wobbly(self, overlap = True):
        ...
        for i in range(n_slices):
            if i % 100 == 0:
                print(f'Generating overlay slice {i:d}/{n_slices:d}!')
...
def align_layout(layout, axis_range=None, max_shifts=10, axis_mip=None,
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: aligning {len(alignments):d} pairs of wobbly sources.')
    ...
    if verbose:
        timer.print_elapsed_time(f'Alignment: aligning {len(alignments):d} pairs of wobbly sources')
...
def align_wobbly_axis(source1, source2, axis=2, axis_range=None, max_shifts=10, axis_mip=None,
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: wobbly alignment {source1.identifier!r}->{source2.identifier!r} along {axis=:d}')
    ...
    if start > stop:
        raise ValueError(f'The sources do not overlap along {axis=:d}!')
    n_slices = stop - start
    ...
    if stack_validation_params:
        ...
        if verbose and not valid:
            print(f'Alignment: Source {source1.identifier!r} is not valid!')
        if valid:
    ...
    for i, a in enumerate(range(start, stop)):
        if verbose and i % 100 == 0:
            print(f'Alignment: Wobbly alignment {source1.identifier!r}->{source2.identifier!r} '
                  f'along {axis=:d}: slice {i:d} / {a_stop - a_start:d}')
        ...
        if slice_validation_params:
            ...
            if verbose and not valid:
                print(f'Alignment: Slice {a - a_start:d} with coordinate {a:d} in '
                      f'source {source1.identifier!r} is not valid!')
            if valid:
                i2rawa = i2raw[slice2_a]
                valid = _validate(i2rawa, **slice_validation_params)
                if verbose and not valid:
                    print(f'Alignment: Slice {a - a_start:d} with coordinate {a:d} in'
                          f' source {source2.identifier!r} is not valid!')
            if not valid:
    ...
    if verbose:
        timer.print_elapsed_time(f'Alignment: Wobbly slice alignment {source1.identifier!r}->{source2.identifier!r}'
                                 f' along {axis=:d} done')

        if verbose_has_flag(verbose, 'save'):
            filename = verbose.full_filename(f'errors_{source1.identifier!r}_{source2.identifier!r}.npy')
            np.save(filename, errors)
            verbose.save = f'{source1.identifier!r}_{source2.identifier!r}'

    results = _shifts_qualities_status(errors, status, add_shift=shift_min, verbose=verbose, **find_shifts)

    if verbose:
        timer.print_elapsed_time(f'Alignment: Wobbly alignment {source1.identifier!r}->{source2.identifier!r}'
                                 f' along {axis=:d} done')
...
def _prepare(array, method='normalization', **kwargs):
    if method == 'normalization':
        ...
        raise ValueError(f'Preparation method {method!r} not valid!')
...
def validate_foreground(array, valid_range = (800,None), size = None, fraction = None, verbose = True):
    ...
    if size is None:
        valid = foreground > 0
        if verbose and not valid:
            print(f'Alignment: All {array.size:d} pixels are background in range {valid_range!r}!')
    else:
        valid = foreground >= size
        if verbose and not valid:
            print(f'Alignment: Not enough foreground pixels {foreground:d} < {size:d} in range {valid_range!r}!')
...
def _validate(array, method='foreground', **kwargs):
    if method == 'foreground':
        ...
        raise ValueError(f'Validation method {method!r} not valid!')


import skimage.feature as sk_image_feature

def detect_local_minima(error, distance=1):
    minima = sk_image_feature.peak_local_max(-error, min_distance=distance, exclude_border=True)
...
def _detect_minima(array, method='local_minima', **kwargs):
    if method == 'local_minima':
        ...
        raise ValueError(f'Method {method!r} not valid for minima detection!')
...
def shifts_from_tracing(errors, status, cutoff=None, new_trajectory_cost=None, minima='local_minima', verbose=False, **kwargs):
    ...
    for s,e in zip(starts, ends):
        ...
        if verbose.has_flag('save'):
            filename = verbose.full_filename(f'trajectories_{verbose.save}_{s:d}_{e:d}.npy')
            np.save(filename, trajectories)
            filename = verbose.full_filename(f'positions_{verbose.save}_{s:d}_{e:d}.npy')
            np.save(filename, positions)

        # successively add longer trajectories
        ...
        while n_opt < n_measured_se:
            ...
            # remove non-relevant trajectories
            ts = t_opt[-1][0][0]
            te = t_opt[-1][-1][0]
        ...
        if verbose.has_flag('save'):
            ...
            filename = verbose.full_filename(f'trajectory_opt_{verbose.save}_{s:d}_{e:d}.npy')
...
def _shifts_qualities_status(errors, status, method='minimization', add_shift=None, **kwargs):
    ...
    if method == 'minimization':
        ...
        raise ValueError(f'Method {method!r} not a valid for shift detection!')
...
def inspect_align_layout(alignment, verbose):
    """Parse the information saved during an align_layout.
    ...
    verbose = Verbose(verbose)
    verbose.save = f'{alignment.pre.identifier!r}_{alignment.post.identifier!r}'

    #error
    error_file = verbose.full_filename(f'errors_{verbose.save}.npy')
...
def place_layout(layout, min_quality = None, method = 'optimization',
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Placement: placing positions in {n_slices:d} slices!')
    ...
    if verbose:
        timer.print_elapsed_time(f'Placement: placing positions in {n_slices:d} slices done!')
...
def _place_slice(displacements, qualities, status, positions, alignment_pairs, min_quality=-np.inf):
    ...
    for pairs, displacement in zip(component_pairs, component_displacements):
        _place_slice_component(positions, pairs, displacement)
...
def _connected_components(positions, alignment_pairs, displacements):
    ...
    for i in range(n_components):
        ...
        component_displacements.append(displacements)
...
def _place_slice_component(positions, alignment_pairs, displacements, fixed = None):
    ...
    # construct the mappings between node ids and index 1:n_images
...
def _optimize_slice_positions(positions, components, processes = None, workspace=None, verbose = False):
    ...
    # that belong to the connected components of the clusters
    ...
    for cci, cluster_component in enumerate(cluster_components_):
        ...
        if verbose:
            print(f'Placement: optimizing component {cci:d}/{n_components:d} with {n_clusters:d} clusters!')
        ...
        if verbose:
            print(f'Placement: done constructing constraints for component {cci:d}/{n_components:d}!')
        ...
        if verbose:
            print(f'Placement: component {cci:d}/{n_components:d} optimized!')


            # note overall shifts between components is not touched but might be based on
    # keeping overall distance.
    return positions
...
def _straighten_slice_positions(positions, components, tile_positions):
    ...
    #so we can straighten the center tile in each cluster component.
...
def smooth_window(x, window_length = 10, window = 'bartlett', binary = None):
    ...
    if window:
        ...
        if not window in windows:
            raise ValueError(f'Window not in {windows!r}!')
...
def smooth_displacements(displacements, valids, method = 'window', **kwargs):
    ...
    if method == 'window':
        ...
        raise ValueError(f'Smoothing {method=!r} not valid!')
...
def stitch_layout(layout, sink, method = 'interpolation', processes = None, verbose = True, workspace=None):
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Stitching: stitching {n_slices:d} sliced layouts.')
...
def _stitch_slice(slice_layout, slice_id, n_slices, sink, method, axis, full_region, verbose):
    if verbose:
        print(f'Stitching: stitching wobbly slice {slice_id:d}/{n_slices:d}')
...
def _test():
    ...
    shifts, qualities, status, errors, overlaps = results
    stw.strg.dv.plot((errors.transpose([1,2,0]),) + overlaps)
    ...
    for i,s in enumerate(l.sources):
        plt.plot(s.wobble[:,0], label=f'{i:d}')
    plt.legend()
    ...
    #TODO: option to reduce the shape of the overlaps used for alignment to speed things up
