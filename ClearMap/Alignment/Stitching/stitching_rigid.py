...
class Region:
    ...
    def extent(self):
        ...
        Note
        ----
        The extent can differ from the shape in case the source is bent or wobbly.
        """
    ...
    def position_to_local(self, position):
        """Converts a position to a local position / the source's origin.
    ...
    def position_from_local(self, local_position):
        """Converts a position given in local coordinates to the non-local position.
    ...
    def coordinate_to_local(self, coordinate, axis):
        ...
        Returns
        -------
        local_coordinate : tuple of ints, or int
          The local coordinate within this region.
        """
        return coordinate - self.position[axis]

    def coordinate_from_local(self, local_coordinate, axis):
        ...
        """
        return local_coordinate + self.position[axis]
...
class Overlap(Region):
    ...
    def __str__(self):
        ...
        s = f"Overlap[{lower!r}, {upper!r}]<{n_sources!r}>"
        return s
...
class Source(SourceRegion, src.AbstractSource):
    ...
    def slice_along_axis(self, coordinate = None, local_coordinate = None, axis = 2):
        ...
        if not 0 <= local_coordinate < self.shape[axis]:
            raise ValueError(f'The source cannot be sliced at the base coordinate {local_coordinate:d}!')
...
class Layout(SourceRegion, src.AbstractSource):
    ...
    def embedding(self):
        """Splits the set of co-axial sources into a minimal set of non-overlapping regions.
    ...
    ### Alignment, placement and stitching
    ...
    def align_axis(self, depth = 10, max_shifts = 10, axis = None, axis_range = None, clip = None, background = None, processes = None, verbose = False):
        ...
        axis : int
          The axis to align the sources along.
...
def _initialize_tiles_from_sources(sources, tile_shape = None, tile_positions = None,  positions = None, overlaps = None, alignments = None):
    ...
    if len(tile_positions) != len(sources):
        ValueError(f'Number of sources = {len(sources):d} does not match grid positions = {len(tile_positions):d} !')
...
def check_alignments_and_sources(alignments, sources, verbose = False):
    ...
    for i,a in enumerate(alignments):
        if a.pre not in sources:
            if verbose:
                print(f"Alignment {i:d}, pre source {a.pre.id:d} not in sources {[s.id for s in sources]!r}!")
            return False

        if a.post not in sources:
            if verbose:
                print(f"Alignment {i:d}, post source {a.post.id:d} not in sources {[s.id for s in sources]!r}!")
            return False
    return True
...
def connected_components(alignments, sources = None, min_quality = None, with_sources = False):
    ...
    min_quality : float, tuple of floats or None
      The minimal quality for alignments to be included in the calculation.
    with_sources : bool
...
def align_2_sources(src1, src2, max_shifts = 10, clip = None, background = None, normalize = False, verbose = False, debug = False):
    ...
    if src1.ndim != src2.ndim:
        raise ValueError(f'Sources expected to have same dimension, '
                         f'found {src1.ndim:d} and {src2.ndim:d} dimensional images!')
    ndim = src1.ndim
    ...
    if debug or verbose:
        print(f'Alignment: positions = {src1.position!r},{src2.position!r}, '
              f'shapes={src1.shape!r},{src2.shape!r} shifts={shift_min!r},{shift_max!r}')
    ...
    if debug:
        print(f'Alignment: data shapes = {i1.shape!r}, {i2.shape!r}')
        print(f'Alignment: overlapping slices = {slice1!r}, {slice2!r}')
    ...
    if background is not None:
        if isinstance(background, tuple):
            ...
            bad = bb < aa1
            if verbose and bad:
                print(f'Alignment: Not enough pixels {bb:d}<{aa1:d} above background {background[0]:d}'
                      f' in source {src1.tile_position!r}')
            if not bad:
                bb = np.sum(i2 >= background[0])
                bad = bb < aa2
                if verbose and bad:
                    print(f'Alignment: Not enough pixels {bb:d}<{aa2:d} above background {background[0]:d}'
                          f' in source {src2.tile_position!r}')
        else: #background is int
            bad = np.all(i1 < background) or np.all(i2 < background)
            if verbose and bad:
                print(f'Alignment: No good signal between {src1.tile_position!r} and {src2.tile_position!r}')
        if bad:
    ...
    if debug:
        ...
        print(f'Alignment: shapes = {i1.shape!r}, {i2.shape!r}')
    ...
    if verbose:
        print(f'Alignment: done! {shift=!r}, {quality=:.2e}')
...
def align_layout(layout, max_shifts = 10, clip = None, background = None, processes = None, verbose = False):
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: Aligning {layout!r}')
    ...
    if verbose:
        timer.print_elapsed_time(f'Alignment: Aligning {layout!r}')
...
def _align_layout(pre, post, aid, n_alignments, max_shifts, clip, background, verbose):
    if verbose:
        ...
        print(f'Alignment: aligning source {pre.id:d} with {post.id:d},'
              f' alignment pair {aid:d}/{n_alignments:d}')
...
def align_layout_on_tiling(layout, overlaps = 10, max_shifts = 10, clip = None, background = None, processes = None, verbose = False):
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: Aligning {layout!r} on gird!')
    ...
    if verbose:
        timer.print_elapsed_time(f'Alignment: Aligning {layout!r} on grid')
...
def _align_layout_on_tiling(pre, post, axis, aid, n_alignments, overlaps, max_shifts, clip, background, verbose):
    if verbose:
        ...
        print(f'Alignment: aligning source {pre.id:d} with {post.id:d} along {axis=:d},'
              f' alignment pair {aid:d}/{n_alignments:d}')
...
def align_layout_axis(layout, axis = 2, depth = 10, max_shifts = 10, axis_range = None, clip = None, background = None, processes = None, verbose = False):
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: aligning sources in layout along axis={axis:d}!')
...
def _align_layout_axis(src1, src2, aid, n_alignments, axis, axis_range, depth, max_shifts, clip, background, verbose):
    if verbose:
        print(f'Alignment: aligning {src1.tile_position!r} with {src2.tile_position!r} '
              f'along {axis=:d} and range {axis_range!r}, alignment pair {aid:d}/{n_alignments:d} !')
    ...
    if verbose:
        ...
        print(f'Alignment: aligning {src1.tile_position!r} with {src2.tile_position!r}'
              f' along {axis=:d}, alignment pair {aid:d}/{n_alignments:d} done, {shift=!r}, {quality=:.2e}!')
...
def align_layout_rigid_mip(layout, depth=10, max_shifts=10, ranges=None, clip=None, background=None,
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Alignment: rigidly aligning sources in layout: {layout!r}!')
    ...
    if verbose:
        timer.print_elapsed_time(f'Alignment: rigidly aligning sources in layout: {layout!r}!')
...
def _align_layout_ridgid_mip(src1, src2, aid, n_alignments, depth, max_shifts, ranges, clip, background, verbose):
    ...
    if verbose:
        shift, quality = result
        print(
            f'Alignment: aligning {src1.tile_position!r} with {src2.tile_position!r},'
            f' alignment pair {aid:d}/{n_alignments:d} done, {shift=!r}, {quality=:.2e}!')
...
def positions_from_tree(alignments, sources = None, min_quality = None, fixed_source = None, lower_to_origin = False):
    ...
    g, p = connect_sources(alignments, n_sources, source_to_index)

    positions = get_positions_from_tree(g, fixed_source, source_to_index, n_sources, ndim, p)
...
def positions_from_optimization(alignments, sources = None, min_quality = None, fixed_source = None, lower_to_origin = False): # optimize positions !
    ...
    alignments : list of Alignments classes
      The pairwise alignments between images to optimize globally.
    sources : list of Source classes or None
      The sources to optimize the positions for, if None determine from the alignments.
    min_quality : float
      The minimal quality of the alignment.
    fixed_source : Source class, int or None
    ...
    """
    # TODO: use quality measure in optimization
    alignments = filter_alignments(alignments, min_quality = min_quality)
...
def place_layout(layout, method = 'optimization', min_quality = None, lower_to_origin = False, verbose = False):
    ...
    if method not in methods:
        ValueError(f'Method {method!r} not in {methods!r}')

    if verbose:
        timer = tmr.Timer()
        print(f'Placement: placing {layout!r}!')
...
def embedding(sources, shape = None, position = None):
    """Splits a set of co-axial sources into a minimal set of non-overlapping regions.
...
def _add_overlap_region(regions, region):
    ...
    # try to add the full new region first
    regsadd = [region]
    ...
    while len(regscheck) > 0 and len(regsadd) > 0:
        ...
        for a in range(len(regsadd)):
            ...
            if ov is not None:
                ...
                # add all non-overlapping regions to the 'to be added' list
...
def stitch_weights(shape):
    ...
    -------
    weights : array
      The weights for the pixels of the source.
    """
...
def stitch_by_function_with_weights(layout, sink = None, function = np.sum, weight_function = stitch_weights):
    ...
    for r in regions:
        ...
        if n_sources > 1:
            rd = np.zeros((n_sources,) + r.shape)
            wd = np.zeros((n_sources,) + r.shape)
...
def _stitching_function_from_method(method):
    ...
    if method not in method_map.keys():
        ValueError(f'Method {method!r} not in {method_map.keys()!r}')
...
def stitch_layout(layout, sink = None, method = 'interpolation', verbose = False):
    ...
    if verbose:
        timer = tmr.Timer()
        print(f'Stitching: Stitching {layout!r} with method {method}.')

    function = _stitching_function_from_method(method)
    result = function(layout, sink = sink)

    if verbose:
        timer.print_elapsed_time(f'Stitching: Stitching {layout!r} with method {method}')
