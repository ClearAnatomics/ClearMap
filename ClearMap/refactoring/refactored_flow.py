...
def load_graph(work_dir, control):
    ...
    """
    try:
        graph = ggt.load(os.paht.join(work_dir, control, 'data_graph_correcteduniverse.gt'))
    except:
        graph = ggt.load(os.path.join(work_dir, control, f'{control}_graph.gt'))
...
def load_sampledict(work_dir, control):
    ...
    """
    try:
        with open(os.path.join(work_dir, control, f'sampledict{control}.pkl', 'rb') as fp:
...
def set_vertex_properties(graph):
    ...
    """
    try:
        from_e_prop2_vprop(graph, 'artery')
        from_e_prop2_vprop(graph, 'vein')
    except:
...
def get_filtered_graph(graph, label, mode='arteryvein'):
    ...
    order, level = 1006, 3  # @Sophie: magic numbers
    ...
    order, level = 463, 6  # @Sophie: magic numbers
    label_leveled = ano.convert_label(label, key='order', value='order', level=level)
...
def get_edge_vector(graph, control, normed=False, oriented=True, criteria='distance'):
    x = graph.vertex_coordinates()[:, 0]  # @Sophie: see if could use method from main stock of functions
    y = graph.vertex_coordinates()[:, 1]
    z = graph.vertex_coordinates()[:, 2]
    if criteria=='pressure':
        try:
            graph.vertex_property('pressure')
        except:
            f, v = compute_flow_f_schmid(work_dir, graph,control)
