...
class DataViewer(pg.QtGui.QWidget):
  def __init__(self, source, axis = None, scale = None, title = None, invertY = False, minMax = None, screen = None, parent = None, max_projection = None, *args):
    ### Images soures
    ...
    self.max_projection_edit = pg.QtGui.QLineEdit()
    #self.max_projection_edit.setValidator(pg.QtGui.QIntValidator())
    self.max_projection_edit.setMaxLength(4)
    self.max_projection_edit.setAlignment(pg.QtCore.Qt.AlignRight)
    ...
    axis_tools_layout.addWidget(self.source_label,0,4);
  ...
  def updateSlice(self):
    ...
    #print(index, self.source_index);
    if self.max_projection is not None:
      slc_ax = (slice(max(0,index - self.max_projection), min(self.source_shape[ax], index + self.max_projection)),);
    else:
      slc_ax = (index,);
    #print(slc_ax)
  ...
  def updateImage(self):
    ...
    for i, s in zip(self.image_items, self.sources):
      ...
      #print(self.source_slice)
      #print(self.max_projection)
      #print(self.source_shape);
      #print(self.source_scale)
      
      if self.max_projection is not None:
        image = np.max(s[self.source_slice[:s.ndim]], axis=self.source_axis);
      else:
  ...
  def change_max_projection(self, value = None):
    ...
    self.source_index[self.source_axis] += 1; # to trigger update
    self.updateSlice();
    self.updateImage();
