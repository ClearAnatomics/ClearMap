...
class LUT(pg.QtGui.QWidget):
  def __init__(self, image = None, color = 'red', percentiles = [[-100,0,50],[50,100,150]], parent = None, *args):
...
class DataViewer(pg.QtGui.QWidget):
  def __init__(self, source, axis = None, scale = None, title = None, invertY = False, minMax = None, parent = None, *args):
    ### Images soures
...
def plot(source, axis = None, scale = None, title = None, invertY = True, minMax = None):
  if isinstance(source, (list, tuple)):
    return multi_plot(source, axis = axis, scale = scale, title = title, invertY = invertY, minMax = minMax);
  else:          
...
def dualPlot(source1, source2, axis = None, scale = None, title = None, invertY = True, minMax = None, arrange = True, percent = 80):
  return multiPlot([source1, source2], axis = axis, scale = scale, title = title, invertY = invertY, minMax = minMax);
 

def multiPlot(sources, axis = None, scale = None, title = None, invertY = True, minMax = None, arrange = True):
  
  dvs = [DataViewer(source = s, axis = axis, scale = scale, title = title,
                    invertY = invertY, minMax = minMax) for s in sources];

  if arrange:
    geo = guiutil.tiledLayout(len(dvs), percent = 80);
