...
class CellDetector(TabProcessor):
    def __init__(self, sample_manager=None, channel=None, registration_processor=None):
        ...
        self.workspace = None
        self.channel = None
        self.cell_detection_re = (
            "Processing block",
            re.compile(r".*?Processing block \d+/\d+.*?\selapsed time:\s\d+:\d+:\d+\.\d+"),
        )
        if channel is None:
            raise ValueError(
                f"No channel specified. Please provide a channel name "
    ...
    def plot_voxelized_counts(self, arrange=True, parent=None):
        ...
        return plot_3d.plot(
            self.get_path("density", channel=self.channel, asset_sub_type="counts"),
            scale=scale,
    ...
    def get_voxelization_params(self, sub_step=""):
        voxelization_parameter = {"radius": self.processing_config["voxelization"]["radii"], "verbose": True}
        if self.workspace.debug:  # Path will use debug
            ...
            voxelization_parameter["shape"] = self.get(
                "atlas", channel=self.channel, asset_sub_type="annotation"
            ).shape()
        else:
    ...
    def voxelize_weighted(self, coordinates, source, voxelization_parameter):
        ...
        voxelization.voxelize(
            coordinates, sink=intensities_file_path, weights=intensities, **voxelization_parameter
        )  # WARNING: prange
    ...
    def atlas_align(self):
        ...
        if self.registration_processor.was_registered:  # FIXME: check if should be registered and raise error
            ...
            channel_cfg = self.registration_processor.config["channels"][
                self.sample_manager.alignment_reference_channel
            ]
            ...
            warnings.warn(
                "Atlas alignment requires a registered sample." "Skipping alignment and using resampled coordinates."
            )
    ...
    def transform_coordinates(self, coords):
        ...
        if self.registration_processor.was_registered:
            for i, channel in enumerate(self.get_registration_sequence_channels(stop_channel=target_channel)):
                if self.registration_processor.config["channels"][channel]["moving_channel"] in (
                    None,
                ...
                coords = elastix.transform_points(
                    coords, transform_directory=results_dir, binary=USE_BINARY_POINTS_FILE
                )
    ...
    def filter_cells(self):
        ...
        cell_detection.filter_cells(
            source=self.get_path("cells", channel=self.channel, asset_sub_type="raw"),
    ...
    def run_cell_detection(self, tuning=False, save_shape=False, save_as_binary_mask=False):
        ...
        cell_detection_param["background_correction"]["shape"] = self.processing_config["detection"][
        ...
        try:
            ...
            )  # WARNING: prange inside multiprocess (including arrayprocessing and devolvepoints for vox)
    ...
    def export_as_csv(self):
        ...
        """
        warnings.warn(
            "export_as_csv is deprecated and will be removed in future versions;"
    ...
    def export_collapsed_stats(self, all_regions=True):
        ...
        if all_regions:  # Add regions even if they are empty
            ...
            vol_map = annotator.get_lateralised_volume_map(
                self.registration_processor.config["channels"][self.sample_manager.alignment_reference_channel][
            ...
            collapsed = tmp.merge(
                collapsed[["Structure ID", "Hemisphere", "Cell counts", "Average cell size"]],
                how="left",
                on=["Structure ID", "Hemisphere"],
            )
    ...
    def plot_cells_3d_scatter_w_atlas_colors(self, raw=False, parent=None):
        ...
        if not asset.exists or not self.df_path.exists:
            raise MissingRequirementException(
                f"plot_cells_3d_scatter_w_atlas_colors missing files:"
                f'image: {asset.path} {"not" if not asset.exists else ""} found'
                f'cells data frame {"not" if not self.df_path.exists else ""} found'
            )
        dv = qplot_3d.plot(
            asset.path,
            title=f'{asset_properties["asset_type"].title()} and cells',  # FIXME: correct scaling for anisotropic if raw
            arrange=False,
            lut="white",
            parent=parent,
        )[0]
        ...
        if "hemisphere" in df.columns:
            ...
            hemispheres = None
        dv.scatter_coords = Scatter3D(
            coordinates, colors=df["color"].values, hemispheres=hemispheres, half_slice_thickness=0
        )
        dv.refresh()
        return [dv]
    ...
    def plot_filtered_cells(self, parent=None, smarties=False):
        ...
        dv = qplot_3d.plot(
            stitched_path, title="Stitched and filtered cells", arrange=False, lut="white", parent=parent
        )[0]
        scatter = pg.ScatterPlotItem()
    ...
    def plot_background_subtracted_img(self):
        ...
        return plot_3d.plot_3d(
            self.get_path("stitched", channel=self.channel), view=p, cmap=plot_3d.grays_alpha(alpha=1)
        )
    ...
    def export_to_clearmap1_fmt(self):
        ...
        """
        warnings.warn(
            "export_to_clearmap1_fmt is deprecated and will be removed in future versions;"
            "please use the new formats from atlas_align and export_collapsed_stats",
            DeprecationWarning,
            2,
        )
    ...
    def convert_cm2_to_cm2_1_fmt(self):
        ...
        color_map = {
            lbl: annotator.find(lbl, key="order")["rgb"] for lbl in unique_labels
        }  # WARNING RGB upper case should give integer but does not work
        ...
        atlas_scale = self.registration_processor.config["channels"][self.sample_manager.alignment_reference_channel][
            "resampled_resolution"
        ]
        atlas_scale = np.prod(atlas_scale)
        volumes = {
            _id: (atlas == _id).sum() * atlas_scale for _id in id_map.values()
        }  # Volumes need a lookup on ID since the atlas is in ID space
