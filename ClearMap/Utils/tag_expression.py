...
class Tag(object):
    ...
    def parse(self, tag):
        ...
        if len(values) > 3:
            raise ValueError(f'Found {len(values)} > {3} tag specifications!')
        ...
        for i, v in enumerate(values):
            try:
                if i > 0:  # don't expect a width specification in the first entry
                    tag_attributes['width'].append(int(v))
            except ValueError:
                if v in [TAG_INT, TAG_STR]:
                    tag_attributes['ttype'].append(v)
                elif not v:
                    raise ValueError('Found two tag separators without a value in between!')
                else:
                    tag_attributes['name'].append(v)

        for key, value in tag_attributes.items():
            if len(value) > 1:
                raise ValueError(f'More than one {key} found in tag: {value}')
            tag_attributes[key] = value[0] if len(value) == 1 else None
...
class Expression(object):
    ...
    def parse(self, expression):
        p = re.compile(f'{TAG_START}.*?{TAG_END}')
        pattern = []
        tags = []
        start = 0
    ...
    def __str__(self):
        return f'TagExpression({self.tag()})'
