...
sys.path.insert(0, '/home/xxx/ClearMap2/')

# %%


def fix_tiff_file_name(tiff_older, channel_name):
    tiffs = glob.glob(os.path.join(tiff_older, channel_name, "*.tif"))
    for tif in tiffs:
        dir_name = os.path.dirname(tif)
        nums = os.path.basename(tif).split('_')
        fix_num = str(
            int(int(nums[-1].replace('.tif', ''))/20)).zfill(6) + '.tif'
        nums[-1] = fix_num
        dest_file = os.path.join(dir_name, '_'.join(nums))
        shutil.move(tif, dest_file)
        #print(tif, dest_file)
...
if __name__ == "__main__":
    ...
    directory = '/path/to/Brain_x_Autofluor_NeuN_cFos'

    # %%
    # The LifeCanvas dataset named the tiff file name with
    # x20 for the z slice number
    #fix_tiff_file_name(directory, 'Ex_488_Em_525_stitched')
    #fix_tiff_file_name(directory, 'Ex_647_Em_690_stitched')

    # %%
    expression_raw = 'Ex_488_Em_525_stitched/068930_114470_<Z,6>.tif'
    expression_auto = 'Ex_647_Em_690_stitched/068930_114470_<Z,6>.tif'
    ...
    res.resample(ws.filename('stitched'), sink=ws.filename(
        'resampled'), **resample_parameter)
    ...
    res.resample(ws.filename('autofluorescence'), sink=ws.filename(
        'resampled', postfix='autofluorescence'), **resample_parameter_auto)
    ...
    cell_detection_parameter['background_correction']['save'] = ws.filename(
        'cells', prefix='bgremove')

    cell_detection_parameter['intensity_detection']['measure'] = ['source']
    cell_detection_parameter['shape_detection']['threshold'] = 1000

    #io.delete_file(ws.filename('cells', postfix='maxima'))
    cell_detection_parameter['maxima_detection']['shape'] = 3
    cell_detection_parameter['maxima_detection']['threshold'] = 700
    cell_detection_parameter['maxima_detection']['save'] = ws.filename(
        'cells', postfix='maxima')

    processing_parameter = cells.default_cell_detection_processing_parameter.copy()
    processing_parameter.update(
        processes=4,  # 'serial', # 'serial',
        size_max=100,  # 20, #100, #35,
        size_min=50,  # 11,# 30, #30,
        overlap=16,  # 10, #32, #10,
        verbose=True
    )
    ...
    coordinates = np.hstack(
        [ws.source('cells', postfix='raw')[c][:, None] for c in 'xyz'])
    ...
    coordinates = np.array(
        [ws.source('cells', postfix='filtered')[c] for c in 'xyz']).T
    ...
    cells_data = rfn.merge_arrays(
        [source[:], coordinates_transformed, label, names], flatten=True, usemask=False)
    ...
    np.savetxt(ws.filename('cells', extension='csv'),
               source[:], header=header, delimiter=',', fmt='%s')
    ...
    vox.voxelize(coordinates, sink=ws.filename(
        'density', postfix='counts'), **voxelization_parameter)
    ...
    vox.voxelize(coordinates, sink=ws.filename(
        'density', postfix='intensities'), **voxelization_parameter)
