...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_points(const source_t[:, :, :] source, const kernel_t[:, :, :] kernel,
                              const index_t[:, :] points,
                              sink_t[:] sink, int processes) noexcept nogil:
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_points_no_check(const source_t[:, :, :] source, const kernel_t[:, :, :] kernel,
                                       const index_t[:, :] points,
                                       sink_t[:] sink, int processes) noexcept nogil:
    """Convolves binary data with a specified kernel at specific points only."""
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_indices(const source_t[:] source, const index_t[:] strides,
                               const kernel_t[:, :, :] kernel, const index_t[:] points,
                               sink_t[:] sink, int processes) noexcept nogil:
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_indices_no_check(const source_t[:] source, const index_t[:] strides,
                                        const kernel_t[:, :, :] kernel, const index_t[:] points,
                                        sink_t[:] sink, int processes) noexcept nogil:
    """Convolves binary data with a specified kernel at specific points given as indices of a flat array."""
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_indices_if_smaller_than(const source_t[:] source, const index_t[:] strides,
                                               const kernel_t[:, :, :] kernel, const index_t[:] points,
                                               max_t max_value, bool_t[:] sink, int processes) noexcept nogil:
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void convolve_3d_indices_if_smaller_than_no_check(const source_t[:] source, const index_t[:] strides,
                                                        const kernel_t[:, :, :] kernel, const index_t[:] points,
                                                        max_t max_value, bool_t[:] sink, int processes) noexcept nogil:
    """Convolves binary data with a specified kernel at specific points given as indices and check if the result is smaller than a maximal value."""
