...
cpdef void prepare_temp(const source_t[:] source, temp_t[:] temp,
...
cpdef void label_temp(temp_t[:] temp, const index_t[:] strides,
                      const index_t[:] seeds, int processes) noexcept nogil:
    ...
    cdef const index_t* strides_pointer = &strides[0]
    cdef const index_t* seeds_pointer = &seeds[0]
...
cdef void label_from_seed(temp_t* temp, index_t size, index_t seed,
                          const index_t* strides, index_t ndim) noexcept nogil:
...
cpdef void fill(const source_t[:] source, const temp_t[:] temp, sink_t[:] sink,
                int processes, bint verbose=False) noexcept nogil:
    cdef index_t n = source.shape[0]
    cdef index_t i
