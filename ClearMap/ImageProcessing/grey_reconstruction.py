...
`CellProfiler <https://www.cellprofiler.org>`_.
...
def reconstruct(seed, mask=None, method='dilation', selem=None, offset=None):
    ...
    if seed.shape != mask.shape:
        raise ValueError(f'Seed shape {seed.shape} and mask shape {mask.shape} do not match')
    ...
    if method == 'dilation':
        index_sorted = np.flip(index_sorted)
